...
import type { DeleteQuery, GetQuery, InferQueryRelations, PatchQuery, PostQuery, SelectorDelete, SelectorGet, SelectorPatch, SelectorPost } from "./query.ts";
...
export declare class BepaloQuery<Schema extends Record<string, any>, Database extends {
    query: any;
}> {
    constructor();
    Get<N extends keyof Schema>(options: Omit<GetQuery, "select"> & {
        select?: BepaloQueryWith<SelectorGet, Database, Schema, N>;
    }): URLSearchParams;
    Post<N extends keyof Schema>(options: Omit<PostQuery, "select"> & {
        select?: BepaloQueryWith<SelectorPost, Database, Schema, N>;
    }): URLSearchParams;
    Patch<N extends keyof Schema>(options: Omit<PatchQuery, "select"> & {
        select?: BepaloQueryWith<SelectorPatch, Database, Schema, N>;
    }): URLSearchParams;
    Delete<N extends keyof Schema>(options: Omit<DeleteQuery, "select"> & {
        select?: BepaloQueryWith<SelectorDelete, Database, Schema, N>;
    }): URLSearchParams;
}
export declare const createQuery: <Schema extends Record<string, any>, Database extends {
    query: any;
}>() => BepaloQuery<Schema, Database>;
export {};
